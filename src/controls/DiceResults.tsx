...
export function DiceResults({
  ...
  const dice = useMemo(() => diceRoll.dice.filter(isDie), [diceRoll]);

  const finalValue = useMemo(() => {
    return getCombinedDiceValue(diceRoll, rollValues);
  }, [diceRoll, rollValues]);

  const whiteRolls = useMemo(
    () =>
      dice
        .filter((d) => d.style === "DSD6")
        .map((d) => rollValues[d.id])
        .filter((value): value is number => typeof value === "number"),
    [dice, rollValues]
  );

  const redRolls = useMemo(
    () =>
      dice
        .filter((d) => d.style === "DSD6_RED")
        .map((d) => rollValues[d.id])
        .filter((value): value is number => typeof value === "number"),
    [dice, rollValues]
  );
  ...
  return (
    <Stack alignItems="center" maxHeight="calc(100vh - 100px)">
      {(whiteRolls.length > 0 || redRolls.length > 0) && (
        <Stack spacing={1} sx={{ px: 1, mb: 1 }}>
          {whiteRolls.length > 0 && (
            <Stack direction="row" spacing={1} flexWrap="wrap" justifyContent="center">
              {whiteRolls.map((value, index) => (
                <Typography
                  key={`w-${value}-${index}`}
                  lineHeight="28px"
                  sx={{
                    color: value === 6 ? "success.main" : "white",
                    minWidth: "24px",
                    textAlign: "center",
                    px: 0.75,
                    borderRadius: "8px",
                    backgroundColor:
                      value === 6 ? "rgba(56, 142, 60, 0.18)" : "rgba(255,255,255,0.08)",
                  }}
                >
                  {value}
                </Typography>
          ...
          {redRolls.length > 0 && (
            <Stack direction="row" spacing={1} flexWrap="wrap" justifyContent="center">
              {redRolls.map((value, index) => (
                <Typography
                  key={`r-${value}-${index}`}
                  lineHeight="28px"
                  sx={{
                    color: value === 6 ? "success.main" : "#ff6b6b",
                    minWidth: "24px",
                    textAlign: "center",
                    px: 0.75,
                    borderRadius: "8px",
                    backgroundColor:
                      value === 6 ? "rgba(56, 142, 60, 0.18)" : "rgba(255,255,255,0.04)",
